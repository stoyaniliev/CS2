...
Source control and pipelines are on GitHub with GitHub Actions. Pipelines run on a self-hosted runner, as required.

The repository is organised so that each concern is separately reviewable:

```
terraform/ infrastructure definitions and modules
soar/ collector, rule engine, response actions, playbooks
observability/ monitoring stack deployment
scripts/ test and operational tooling
