...
def dispatch(action: dict, event: dict, playbook: dict) -> bool:
    ...
    try:
        ...
        log.info("Dispatched %s for %s (%s)", action["type"], event.get("event_id", "no-id"), playbook["id"])
...
def process(event: dict):
    ...
    # Events normally arrive from the collector, which always assigns an id.
    # A replayed or hand-crafted message may not have one, and a KeyError here
    # would fail the whole SQS batch rather than the single bad message.
    event_id = event.get("event_id", "no-id")

    for playbook in PLAYBOOKS["playbooks"]:
        matched, reason = matches(playbook, event)
        if not matched:
            log.debug("%s did not match %s: %s", playbook["id"], event_id, reason)
            continue

        matched_any = True
        log.info("Event %s matched %s (%s)", event_id, playbook["id"], playbook["name"])
